...
display_menu() {
  ...
  download_if_script_missing() {
    ...
    if command -v jq &>/dev/null; then
      ...
      url=$(grep -Po '"name":\s*"'$option_name'".*?"gist_URL":\s*"\K[^"]*' "$config_file")
    fi
  ...
  display_submenu() {
    ...
    if command -v jq &>/dev/null; then
      description=$(jq -r --arg name "$option_name" '.options[] | select(.name == $name) | .description' "$config_file")
    else
      description=$(grep -Po '"name":\s*"'$option_name'".*?"description":\s*"\K[^"]*' "$config_file")
    fi
...
read_options_from_config() {
  ...
  # Read option names from config.json using jq or grep/awk
  if command -v jq &>/dev/null; then
    ...
    while IFS= read -r name; do
      temp_options+=("$name")
    done < <(grep -Po '"name":\s*"\K[^"]*' "$config_file")
  fi

  # Assign to initial_options array
  initial_options=("${temp_options[@]}")

  # Append "Clean Up" and "Quit" options separately
